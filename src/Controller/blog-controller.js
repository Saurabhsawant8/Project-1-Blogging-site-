...
const createBlog = async function (req, res) {
    try {
        ...
        if (!userExist) {
            res.status(404).send({ status: false, msg: "Author does not exist" })
        } else {
...
const getBlogs = async function (req, res) {
    try {
        let query= req.query
        const { category, subcategory, tags, authorId } =query
        ...
        const blog = await blogModel.find({isPublished : true ,isDeleted : false,...query} )
...
const deleteblogsByQuery = async function (req, res) {
    ...
    {$set :{isDeleted : true , deletedAt : Date.now() ,isPublished : false}} ,{new : true} )         
